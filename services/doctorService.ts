...
function buildDoctorIdentity(user: any) {
...
export async function ensureDoctorClinicSetup(user: any): Promise<DoctorBootstrapResult> {
    if (!user?.id) {
    ...
    const { data: createdClinic, error: createError } = await (supabase as any)
        .from('clinics')
