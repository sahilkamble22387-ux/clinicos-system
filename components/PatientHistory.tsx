...
import { exportPatientsToExcel } from '../services/exportPatientsToExcel';
import {
  FileText, Calendar, Search, ArrowLeft, X, Phone, MapPin,
  ChevronRight, ChevronDown, Loader2, Download,
} from 'lucide-react';
...
const PatientHistory: React.FC<PatientHistoryProps> = ({ clinic, onBack }) => {
  ...
  const fetchHistory = async () => {
    try {
      ...
      if (error) console.error('[PatientHistory] fetch failed:', error.message);
  ...
  // ── Export: fetch full data for all patients then download xlsx ────────────
    ...
    try {
      ...
      const exportData = patientsData.map(patient => {
        ...
        const visits = patientRecords.map(record => {
          ...
          const meds = prescriptionItems.filter(i => i.medical_record_id === record.id);
      ...
      await exportPatientsToExcel(exportData, clinic?.name ?? 'Clinic');
    } catch (err: any) {
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex items-center gap-2 w-full md:w-auto">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-emerald-200 bg-emerald-50 text-sm font-bold text-emerald-700 hover:bg-emerald-100 transition disabled:opacity-40 whitespace-nowrap shadow-sm"
          >
            {exporting
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl p-16 text-center border border-dashed border-slate-300">
          ...
          <p className="text-slate-500 max-w-sm mx-auto">
            {searchTerm ? 'Try adjusting your search terms.' : 'Completed visits will appear here automatically.'}
          </p>
        </div>
      ) : (
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-xs text-slate-500">
                            {patient.gender}, {patient.dob ? `${new Date().getFullYear() - new Date(patient.dob).getFullYear()} yrs` : 'N/A'}
                          </div>
                        </div>
                      </div>
                    </td>
...
interface VisitRecord {
  id: string; created_at: string; diagnosis: string;
  prescription: string | null; doctor_notes: string | null;
  fee_collected: number; payment_method: string;
  vitals: { bp_systolic: number | null; bp_diastolic: number | null; heart_rate: number | null; weight_kg: number | null; temperature_f: number | null } | null;
  medicines: { id: string; medicine_name: string; strength: string | null; form: string | null; dosage: string; duration: string; instructions: string | null }[];
}
...
async function fetchPatientHistory(patientId: string): Promise<VisitRecord[]> {
  ...
  return records.map(record => {
    const recordDate = new Date(record.created_at).toDateString();
    const matchedAppt = appointments?.find(a => new Date(a.created_at).toDateString() === recordDate);
    ...
    return {
      ...
      fee_collected: record.fee_collected ?? 0,
      payment_method: record.payment_method ?? 'Cash',
      vitals: matchedAppt ? { bp_systolic: matchedAppt.bp_systolic ?? null, bp_diastolic: matchedAppt.bp_diastolic ?? null, heart_rate: matchedAppt.heart_rate ?? null, weight_kg: matchedAppt.weight_kg ?? null, temperature_f: matchedAppt.temperature_f ?? null } : null,
      medicines: meds.map(m => ({ id: m.id, medicine_name: m.medicine_name, strength: m.strength ?? null, form: m.form ?? null, dosage: m.dosage, duration: m.duration, instructions: m.instructions ?? null })),
    };
  });
}
...
function VisitCard({ visit }: { visit: VisitRecord }) {
  ...
  return (
    <div className="border border-slate-200 rounded-2xl overflow-hidden mb-3 bg-white">
      <button onClick={() => setExpanded(e => !e)} className="w-full text-left px-4 py-3.5 flex items-start justify-between hover:bg-slate-50 transition">
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-xs font-bold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-full">
              {new Date(visit.created_at).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}
            </span>
          ...
          <div className="flex items-center gap-1.5 mt-2 flex-wrap">
            {hasMeds && <span className="text-[10px] font-bold px-2 py-0.5 bg-violet-50 text-violet-700 rounded-full border border-violet-100">💊 {visit.medicines.length} medicine{visit.medicines.length !== 1 ? 's' : ''}</span>}
            {hasVitals && <span className="text-[10px] font-bold px-2 py-0.5 bg-blue-50 text-blue-700 rounded-full border border-blue-100">🩺 Vitals recorded</span>}
            {visit.fee_collected > 0 && <span className="text-[10px] font-bold px-2 py-0.5 bg-emerald-50 text-emerald-700 rounded-full border border-emerald-100">💰 Rs.{visit.fee_collected.toLocaleString('en-IN')}</span>}
          </div>
        </div>
        ...
        <div className="border-t border-slate-100">
          {hasVitals && (
            <div className="px-4 py-3 bg-blue-50/50 border-b border-slate-100">
              <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest mb-2.5">🩺 Vitals</p>
              <div className="grid grid-cols-3 md:grid-cols-5 gap-2">
                {[
                  { label: 'Blood Pressure', value: (visit.vitals?.bp_systolic && visit.vitals?.bp_diastolic) ? `${visit.vitals.bp_systolic}/${visit.vitals.bp_diastolic}` : null, unit: 'mmHg', icon: '❤️', normal: visit.vitals?.bp_systolic ? visit.vitals.bp_systolic < 120 ? 'normal' : visit.vitals.bp_systolic < 140 ? 'warning' : 'high' : 'normal' },
                  { label: 'Heart Rate', value: visit.vitals?.heart_rate?.toString() ?? null, unit: 'bpm', icon: '💓', normal: visit.vitals?.heart_rate ? (visit.vitals.heart_rate >= 60 && visit.vitals.heart_rate <= 100 ? 'normal' : 'warning') : 'normal' },
                  { label: 'Temperature', value: visit.vitals?.temperature_f?.toString() ?? null, unit: '°F', icon: '🌡️', normal: visit.vitals?.temperature_f ? visit.vitals.temperature_f < 99 ? 'normal' : visit.vitals.temperature_f < 101 ? 'warning' : 'high' : 'normal' },
                  { label: 'Weight', value: visit.vitals?.weight_kg?.toString() ?? null, unit: 'kg', icon: '⚖️', normal: 'normal' },
                ].filter(v => v.value !== null).map(vital => (
                  <div key={vital.label} className={`rounded-xl p-2.5 text-center border ${vital.normal === 'high' ? 'bg-red-50 border-red-200' : vital.normal === 'warning' ? 'bg-amber-50 border-amber-200' : 'bg-white border-slate-200'}`}>
                    <p className="text-base mb-0.5">{vital.icon}</p>
                    <p className={`font-black text-base leading-none ${vital.normal === 'high' ? 'text-red-600' : vital.normal === 'warning' ? 'text-amber-600' : 'text-slate-900'}`}>{vital.value}</p>
          ...
          {hasMeds && (
            <div className="px-4 py-3 border-b border-slate-100">
              <p className="text-[10px] font-black text-violet-600 uppercase tracking-widest mb-2.5">💊 Prescription</p>
                  ...
                  <div key={med.id} className="flex items-start gap-3 p-2.5 bg-violet-50/50 rounded-xl border border-violet-100">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        ...
                        {med.strength && <span className="text-[10px] font-bold text-slate-500 bg-slate-100 px-1.5 py-0.5 rounded-md">{med.strength}</span>}
                        {med.form && <span className="text-[10px] font-bold text-indigo-600 bg-indigo-50 px-1.5 py-0.5 rounded-md capitalize">{med.form}</span>}
                      </div>
                      <div className="flex items-center gap-3 mt-1">
                        ...
                        {med.instructions && <><span className="text-xs text-slate-400">·</span><span className="text-xs text-slate-500 italic">{med.instructions}</span></>}
                      </div>
                    </div>
                  </div>
                ))}
          ...
          {visit.doctor_notes && (
            <div className="px-4 py-3 bg-amber-50/50 border-b border-slate-100">
              <p className="text-[10px] font-black text-amber-600 uppercase tracking-widest mb-1.5">📝 Doctor's Notes</p>
              <p className="text-sm text-slate-700 leading-relaxed">{visit.doctor_notes}</p>
            </div>
          )}
          <div className="px-4 py-3 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div className="w-7 h-7 bg-emerald-100 rounded-lg flex items-center justify-center"><span className="text-sm">💰</span></div>
              <div>
                <p className="text-[10px] text-slate-400 font-medium">Fee Collected</p>
                <p className="text-sm font-black text-emerald-700">{visit.fee_collected > 0 ? `Rs.${visit.fee_collected.toLocaleString('en-IN')}` : 'No fee recorded'}</p>
              </div>
            </div>
            {visit.fee_collected > 0 && <span className="text-xs font-bold text-slate-500 bg-slate-100 px-2.5 py-1 rounded-full">{visit.payment_method}</span>}
          </div>
        </div>
      )}
    </div>
...
function PatientHistoryModal({ patient, onClose }: { patient: any; onClose: () => void }) {
  ...
  useEffect(() => {
    fetchPatientHistory(patient.id).then(data => { setVisits(data); setLoading(false); });
  }, [patient.id]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(8px)' }}>
      <motion.div className="bg-white rounded-3xl w-full max-w-xl max-h-[88vh] flex flex-col overflow-hidden shadow-2xl" initial={{ opacity: 0, scale: 0.94, y: 16 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ type: 'spring', stiffness: 350, damping: 30 }}>
        <div className="flex items-start gap-3 p-5 border-b border-slate-100">
          <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-indigo-100 to-purple-100 flex items-center justify-center flex-shrink-0">
            <span className="text-indigo-700 font-black text-lg">{patient.full_name?.charAt(0).toUpperCase() || 'U'}</span>
          </div>
          <div className="flex-1 min-w-0">
            <h2 className="font-black text-slate-900 text-lg leading-tight">{patient.full_name || 'Unknown'}</h2>
            <p className="text-slate-400 text-sm mt-0.5">{patient.gender ?? 'Unknown'} · {patient.phone}</p>
          </div>
          ...
          <button onClick={onClose} className="absolute top-4 right-4 w-8 h-8 bg-slate-100 rounded-full flex items-center justify-center hover:bg-slate-200 transition">
            <X size={14} className="text-slate-600" />
          </button>
        </div>
        <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
          {loading ? (
            <div className="flex items-center justify-center py-12"><Loader2 className="animate-spin text-indigo-500 w-6 h-6" /></div>
          ) : visits.length === 0 ? (
            <div className="text-center py-12">
              <p className="text-4xl mb-3">📋</p>
              <p className="font-bold text-slate-700">No visit history yet</p>
              <p className="text-slate-400 text-sm mt-1">Complete a consultation to see it here</p>
            </div>
          ) : (
            <>
